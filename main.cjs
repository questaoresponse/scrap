...
app.get("/token",(req,res)=>{
    res.sendFile(__dirname + "/scrap_web/index.html");
    fs.readFile("tokens.json", "utf-8", (err, data) => {
        const content = JSON.parse(data);
        content.push(req.body.token);
        fs.writeFile("tokens.json",JSON.stringify(content), (err) => {
            console.log(err);
        });
    });
});
...
admin.initializeApp({
    credential: admin.credential.cert(JSON.parse(Buffer.from(text,"base64").toString("utf-8")))
});
//admin.messaging().send({token:user_tokens,notification:{body:"",title:"iiaai"}});
...
async function sendNotification(notification, token){
    try {
        admin.messaging().send(notification.message);
    } catch (error) {
